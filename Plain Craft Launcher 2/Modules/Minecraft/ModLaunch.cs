...
public static class ModLaunch
{
    ...
    private static string[] MsLoginStep1Refresh(string code)
    {
        ...
        {
            // 线程取消必须向上传播，不能吞掉（吞掉会导致 result 为 null，下游 GetJson 抛 JSON 解析异常）
            throw;
        }
        catch (Exception ex)
        {
            // 与原版 PCL 行为一致：识别 RefreshToken 失效的错误关键词，返回 Relogin 触发重新走 Device Code 流程
            if (ex.Message.ContainsF("invalid_grant", true) || ex.Message.ContainsF("must sign in again", true) ||
                ...
                ex.Message.ContainsF("expired", true) ||
    ...
    private static string MsLoginStep2(string accessToken)
    {
        ...
        // 与原版 PCL 一致：若 AccessToken 已以 "d=" 开头则不重复添加前缀
        var rpsTicket = accessToken.StartsWithF("d=") ? accessToken : $"d={accessToken}";
    ...
    private static string MsLoginStep4(string[] tokens)
    {
                   ...
                   .GetResult())
        {
            ...
            if (!response.IsSuccess)
            {
                // 与原版 PCL 一致：根据状态码和响应体匹配特定错误
                if (response.StatusCode == HttpStatusCode.TooManyRequests)
                {
                    ModBase.Log($"正版验证 Step 4 汇报 429：{result}");
                ...
                {
                    ModBase.Log($"正版验证 Step 4 汇报 503：{result}");
                    throw new Exception("$Mojang 服务器出现问题，请稍后再试。\r\n你的网络是正常的，PCL 也是正常的，是 Mojang 出问题了……");
                }
                // #8655：账号封禁
                if (result.Contains("ACCOUNT_SUSPENDED"))
                {
                    ModMain.MyMsgBox(Lang.Text("Minecraft.Launch.Login.Microsoft.Banned"), Lang.Text("Minecraft.Launch.Login.Failed"), Lang.Text("Minecraft.Launch.Login.IKnow"), isWarn: true);
                    throw new Exception("$$");
                }
                ModProfile.ProfileLog("正版验证 Step 4/6 获取 MC AccessToken 失败：" + response.StatusCode);
                response.EnsureSuccessStatusCode();
            }
        }
    ...
    private static string[] MsLoginStep6(string accessToken)
    {
                   ...
                   .GetResult())
        {
            ...
            if (!response.IsSuccess)
            {
                // 与原版 PCL 一致：根据状态码匹配特定错误
                if (response.StatusCode == HttpStatusCode.TooManyRequests)
                {
                    ModBase.Log($"正版验证 Step 6 汇报 429：{result}");
                    throw new Exception(Lang.Text("Minecraft.Launch.Login.Microsoft.TooManyRequests"));
                }
                if (response.StatusCode == HttpStatusCode.NotFound)
                {
                    ModBase.Log($"正版验证 Step 6 汇报 404：{result}");
                    ModBase.RunInNewThread(() =>
                    {
                ...
                ModProfile.ProfileLog("正版验证 Step 6/6 获取玩家档案信息失败：" + response.StatusCode);
                response.EnsureSuccessStatusCode();
            }
        }
