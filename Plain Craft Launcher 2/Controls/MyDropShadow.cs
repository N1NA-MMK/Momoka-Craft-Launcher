...
public class MyDropShadow : Decorator
{
    ...
    // 防御：动画边界条件或配置切换可能让 ShadowRadius 变成负数/NaN/Infinity，
    // 这会导致 OnRender 中 Rect 构造抛出 "宽度和高度必须为非负值" 异常。
    // 使用 CoerceValueCallback 钳制为非负有限值，而非 ValidateValueCallback（后者会抛异常）。
    private static object CoerceShadowRadius(DependencyObject d, object baseValue)
    {
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        if (shadowBounds.Width > 0d && shadowBounds.Height > 0d && color.A > 0)
        {
            ...
            // 防御：当控件尺寸过小或阴影半径过大时，centerWidth/centerHeight 可能为负，
            // 导致 maxRadius 为负，进而使 cornerRadius 变为负值，Rect 构造抛出异常。
            ...
            // 最终防御：理论上 CoerceShadowRadius 已保证 ShadowRadius 非负，
            // 但浮点累加或动画边界条件仍可能产生极小的负值，钳制到 0 以避免 Rect 构造抛异常。
            if (cornerRadius.TopLeft < 0d) cornerRadius.TopLeft = 0d;
            if (cornerRadius.TopRight < 0d) cornerRadius.TopRight = 0d;
            if (cornerRadius.BottomLeft < 0d) cornerRadius.BottomLeft = 0d;
            if (cornerRadius.BottomRight < 0d) cornerRadius.BottomRight = 0d;
