...
public class MyPageRight : AdornerDecorator
{
    ...
    public void TriggerExitAnimation(params FrameworkElement[] elements)
    {
        ...
        var aniList = new List<ModAnimation.AniData>();
        var delay = 0;
        // Apple 对称原则：退出与进入路径镜像、时长匹配。进入 OutBack(350ms) -> 退出 InFluent(280ms)。
        var exitEase = new ModAnimation.AniEaseInFluent(ModAnimation.AniEasePower.Middle);
        foreach (var Element in realElements)
        foreach (var Control in GetAllAnimControls(Element))
            if (Control is MyExtraTextButton)
            {
            ...
            else
            {
                ...
                aniList.Add(ModAnimation.AaOpacity(Control, -1, 180, delay, ease: exitEase));
                aniList.Add(ModAnimation.AaTranslateY(Control, -6, 280, delay, ease: exitEase));
                delay += 20;
            }
        ...
        if (scroll is not null)
        {
            ...
            aniList.Add(ModAnimation.AaTranslateX(scroll, 10d - ((TranslateTransform)scroll.RenderTransform).X, 280, 0,
