...
public partial class FormMain
{
    ...
    private void ShowUpdateLog()
    {
        ModBase.RunInNewThread(() =>
        {
            ...
            if (ModMain.MyMsgBoxMarkdown(changelog,
                ...
                2) ModBase.OpenWebsite("https://github.com/N1NA-MMK/Momoka-Craft-Launcher/releases");
        }, "UpdateLog Output");
    }
    ...
    private void FormMain_Loaded() // (sender As Object, e As RoutedEventArgs) Handles Me.Loaded
    {
        ...
        ModAnimation.AniStart(new[]
        {
            ...
            // 入场位移与旋转：使用稳定的 OutFluent，避免 OutBack 在边界条件下的异常
            ModAnimation.AaDouble(i => TransformPos.Y += (double)i, -TransformPos.Y, 600,
                100, new ModAnimation.AniEaseOutFluent(ModAnimation.AniEasePower.ExtraStrong)),
            ModAnimation.AaDouble(i => TransformRotate.Angle += (double)i,
                -TransformRotate.Angle, 550, 100, new ModAnimation.AniEaseOutFluent(ModAnimation.AniEasePower.ExtraStrong)),
            ModAnimation.AaCode(() =>
            {
        ...
        // 超时保险：使用 DispatcherTimer（异步、不阻塞），若 2.5 秒后动画仍未完成则强制收尾。
        // 不使用 RunInUiWait，避免与动画线程的 UI 调用产生死锁。
        var showTimeout = new DispatcherTimer(DispatcherPriority.Background) { Interval = TimeSpan.FromMilliseconds(2500) };
        showTimeout.Tick += (_, _) =>
        {
            showTimeout.Stop();
            if (isWindowLoadFinished)
                return;
            ModBase.Log("[Animation] 启动动画超时，强制完成窗口初始化", ModBase.LogLevel.Hint);
            TransformPos.Y = 0d;
            if (TransformRotate is not null) TransformRotate.Angle = 0d;
            RenderTransform = null;
            Opacity = Config.Preference.Theme.WindowOpacity / 1000d + 0.4d;
            isWindowLoadFinished = true;
        };
        showTimeout.Start();
        // Timer 启动
            ...
            try
            {
                ...
                if (Environment.GetEnvironmentVariable("PCL_DISABLE_DEBUG_HINT") is null)
                {
                    ...
                    ModMain.MyMsgBox(
                        ...
                        {
                            ModBase.OpenWebsite("https://github.com/N1NA-MMK/Momoka-Craft-Launcher/releases/latest");
                            EndProgram(false);
                        });
                }
        ...
        bool force = true, bool isUpdating = false)
    {
        ...
        if (returnCode == ModBase.ProcessReturnValues.Exception)
        {
            if (!isLogShown)
            {
                ...
                ModBase.Log("请在 https://github.com/N1NA-MMK/Momoka-Craft-Launcher/issues 提交错误报告！");
                isLogShown = true;
    ...
    {
        // On Error Resume Next
        if (sender is FrameworkElement fe && fe.IsMouseDirectlyOver)
            DragMove();
    }
    ...
    private void FormMain_SizeChanged(object? sender = null, EventArgs? e = null)
    {
        ...
        if (PanBack is not null)
        {
            ...
            // 重构后：PanMain 位于嵌套 Grid 的 Row=1，宽高由布局系统自动分配，无需显式设置。
    ...
    {
        // 重构后：logo 区位于固定宽度的左侧主导航栏，不再需要根据标题栏剩余空间动态限制宽度。
        // 保留方法以兼容 XAML 中的 SizeChanged 绑定。
    }
    ...
    /// <summary>
    /// 刷新侧边栏头像：若当前已选定档案且存在本地头像缓存则显示头像，否则显示加号占位。
    /// </summary>
    public void RefreshNavProfile()
    {
        try
        {
            // ImgNavProfile / ProfilePlus 定义在 BtnNavProfile 的 ControlTemplate 内，需通过模板查找
            BtnNavProfile.ApplyTemplate();
            var img = BtnNavProfile.Template.FindName("ImgNavProfile", BtnNavProfile) as MyImage;
            var plus = BtnNavProfile.Template.FindName("ProfilePlus", BtnNavProfile) as FrameworkElement;
            if (img == null || plus == null) return;

            var profile = ModProfile.selectedProfile;
            var hasHead = profile != null && !string.IsNullOrEmpty(profile.SkinHeadId);
            if (hasHead)
            {
